...
internal static partial class Program
{
    ...
    private static int RunEmulator(string[] args, bool isMitigatedChild)
    {
        ...
        // Virtual PS5 HLE exports are registered here, so the runtime can
        var virtualPs5Exports = VirtualPs5Exports.CreateExports(
            Generation.Gen5);

        Console.Error.WriteLine(
            $"[VPS5] Registering {virtualPs5Exports.Count} external HLE exports.");

        foreach (var export in virtualPs5Exports)
        {
            Console.Error.WriteLine(
                $"[VPS5] HLE export: {export.LibraryName}:{export.Name} nid={export.Nid}");
        }
    
        runtimeOptions = runtimeOptions with
        {
            ExternalHleExports = virtualPs5Exports,
        };
        //--
        ...
        out string? logFilePath)
    {
        ...
        runtimeOptions = new SharpEmuRuntimeOptions
        {
            ...
            ExternalHleExports = VirtualPs5Exports.CreateExports(
