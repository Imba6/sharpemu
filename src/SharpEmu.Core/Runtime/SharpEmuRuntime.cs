...
public sealed class SharpEmuRuntime : ISharpEmuRuntime
{
    ...
    public static ISharpEmuRuntime CreateDefault(SharpEmuRuntimeOptions options = default)
    {
        ...
        /*
        * Host runtime exports are registered first intentionally.
        *
        * This allows a compatibility layer such as VirtualPS5 to provide
        * implementations before SharpEmu's built-in fallback exports.
        */
        if (options.ExternalHleExports is { Count: > 0 })
        {
            moduleManager.RegisterExports(
                options.ExternalHleExports);
        }
