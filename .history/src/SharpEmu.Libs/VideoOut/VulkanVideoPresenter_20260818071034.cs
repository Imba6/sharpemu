...
internal static unsafe class VulkanVideoPresenter
{
    ...
    private sealed class Presenter : IDisposable
    {
        ...
        private int _cpuSyncEnabledTraceCount;
        /// <summary>
        ...
        {
            SharpEmu.HLE.GuestImageWriteTracker.FlushFaultDiagnostics();
            var syncEnabled = SharpEmu.HLE.GuestImageWriteTracker.Enabled;
            
                if (Interlocked.Increment(ref _cpuSyncEnabledTraceCount) <= 3)
    {
        Console.Error.WriteLine(
            $"[VPS5][CPUFB] cpu-sync enabled={syncEnabled}");
    }
    ...
    out GuestImageResource? image)
{
    ...
    /*
     * AGC data-format 10 + number-type 9 represents
     * 8-bit RGBA sRGB in the guest image registry.
     *
     * The Vulkan backing format carries the actual
     * R/B component order of the VideoOut buffer.
     */
    var target =
        new GuestRenderTarget(
            address,
     ...
     *
     * That table means the GPU image is known to contain CPU content
     * for the recorded write generation. Sampling the generation after
     * reading guest memory can race a guest CPU write and incorrectly
     * mark newer CPU contents as already uploaded.
     *
     * Normal CPU refresh paths advance the uploaded generation only
     * after the corresponding contents have been validated/staged.
     */
