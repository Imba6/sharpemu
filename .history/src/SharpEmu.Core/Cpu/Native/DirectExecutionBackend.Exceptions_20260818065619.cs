	...
	private unsafe void SetupExceptionHandler()
{
    ...
    var rawHandlerEnabled =
        !string.Equals(
            Environment.GetEnvironmentVariable(
                "SHARPEMU_DISABLE_RAW_HANDLER"),
            "1",
     ...
     * IMPORTANT:
     *
     * AddVectoredExceptionHandler(first: 1, ...) inserts the handler at
     * the HEAD of the VEH chain.
     *
     * The raw handler owns the guest-image protected-page write bridge,
     * so it MUST execute before the full managed VectoredHandler.
     *
     * Therefore:
     *
     *   1. install the normal handler first
     *   2. install the raw handler second with first=1
     *
     * Resulting order:
     *
     *   raw -> normal -> other VEH handlers
     */

    _handlerDelegate = VectoredHandler;
    _handlerHandle = GCHandle.Alloc(_handlerDelegate);

    _exceptionHandlerStub =
    ...
    if (_exceptionHandlerStub == 0)
    {
        throw new InvalidOperationException(
            "Failed to create exception handler trampoline");
    }
    ...
    Console.Error.WriteLine(
        $"[LOADER][INFO] Exception handler installed: " +
        $"0x{_exceptionHandler:X16}");


    /*
     * Install raw handler AFTER the normal handler.
     *
     * Since first=1 prepends it, this guarantees that raw is first in
     * the resulting VEH chain.
     */
    if (rawHandlerEnabled)
    {
        _rawExceptionHandlerStub =
            CreateExceptionHandlerTrampoline(
        ...
        {
            throw new InvalidOperationException(
                "Failed to create raw exception handler trampoline");
        }

        _rawExceptionHandler =
            (nint)AddVectoredExceptionHandler(
                1u,
                _rawExceptionHandlerStub);
        ...
        {
            throw new InvalidOperationException(
                "Failed to install raw exception handler");
        }

        Console.Error.WriteLine(
            $"[LOADER][INFO] Raw exception handler installed: " +
            $"0x{_rawExceptionHandler:X16}");

        /*
         * Pre-JIT the guest-image fault handling machinery before guest
         * execution starts. A real protected-page write must not be the
         * first invocation of this path.
         */
    ...
    {
        Console.Error.WriteLine(
            "[LOADER][INFO] Raw exception handler disabled by " +
            "SHARPEMU_DISABLE_RAW_HANDLER=1");

        /*
         * Keep the tracker path warm even when the raw handler is
         * intentionally disabled; the normal VEH also contains the
         * write-fault fallback.
         */
        SharpEmu.HLE.GuestImageWriteTracker.WarmUp();
    }
     ...
     * Top-level fallback.
     */
    _unhandledFilterDelegate =
        UnhandledExceptionFilter;

    _unhandledFilterHandle =
        GCHandle.Alloc(
            _unhandledFilterDelegate);

    _unhandledFilterStub =
        CreateExceptionHandlerTrampoline(
            Marshal.GetFunctionPointerForDelegate(
                _unhandledFilterDelegate));
    ...
    {
        throw new InvalidOperationException(
            "Failed to create unhandled exception filter trampoline");
    }
