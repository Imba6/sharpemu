...
public sealed class SelfLoader : ISelfLoader
{
        ...
        ISet<string> seenImportNids)
    {
        foreach (var relocation in relocations)
        {
            ...
            /*
            * Some homebrew/toolchain ELFs keep the plain export name in the
            * dynamic symbol table instead of the encoded PlayStation NID.
            *
            * Example:
            *   sceRandomGetRandomNumber -> PI7jIZj4pcE
            */
            if (!string.IsNullOrWhiteSpace(nid) &&
                !Aerolib.Instance.ContainsNid(nid) &&
                Aerolib.Instance.TryGetByExportName(nid, out var catalogSymbol))
            {
                Console.Error.WriteLine(
                    $"[LOADER] Import name resolved to NID: {nid} -> {catalogSymbol.Nid}");

                nid = catalogSymbol.Nid;
            }
